...
export function GitHubWidget({ issues, loading, error }: GitHubWidgetProps) {
  const list = issues ?? [];
  ...
  return (
    <div className={styles.widget}>
      ...
      {loading ? (
        ...
        <span className={styles.dim}>couldn't load</span>
      ) : list.length === 0 ? (
        <span className={styles.dim}>no issues</span>
      ) : (
        <div className={styles.list}>
          {list.slice(0, 3).map((issue) => (
            <div key={issue.number} className={styles.issue}>
              <span className={styles.number}>#{issue.number}</span>
              <span className={styles.issueTitle}>{issue.title}</span>
              {issue.reason && <span className={styles.reason}>{issue.reason}</span>}
            </div>
          ))}
        </div>
      )}
